...
from req import FINISH_ABORT, FINISH_LENGTH, FINISH_MATCHED_TOKEN, Req, ReqStatus
...
@pytest.mark.parametrize("end", [ReqStatus.FINISHED, ReqStatus.FAILED, ReqStatus.ABORTED])
...
def test_waiting_request_can_be_aborted():
    req = make_req()

    req.set_status(ReqStatus.ABORTED)

    assert req.status is ReqStatus.ABORTED
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([ReqStatus.RUNNING, ReqStatus.FINISHED], ReqStatus.ABORTED),  # too late to abort
        ([ReqStatus.ABORTED], ReqStatus.RUNNING),  # aborted never starts
    ],
)
...
def test_abort_finish_reason_json():
    assert FINISH_ABORT().to_json() == {"type": "abort", "message": "Aborted"}
